...
class Agent:
    ...
    def sarsa_learning(self, decay):
        ...
        for episode_num in range(NUM_EPISODES):
            ...
            discounted_rewards.append(total_reward)
        return self.sarsa_table, discounted_rewards
    def get_d_rewards(self, method, num_episodes):
        if method == "q":
            q_vals = self.q_learning(False, num_episodes)
            self.print_policy_q(q_vals) 
            d_rewards = 0
            num_instances = 100
...
vals = []
for i in range(1,10):
    vals.append(agent.get_d_rewards("q", i*1000))
plot(1 + np.array(range(len(vals))), vals, "Plot of num_iter. vs Discounted rewards (q_vals", "Iteration Number", "Discounted Rewards", "Sarsa("+ str(GAMMA) +")")
