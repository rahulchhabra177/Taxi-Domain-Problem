...
epsilon = float(sys.argv[3])         # learning rate
EPISODE_SIZE = 500
...
class State:
    ...
    def get_random_depot_state(self):
        if self.is_10_by_10:
            return self.get_random_depot_state_10()
        r = pair(0,0)
        g = pair(0,4)
        y = pair(4,0)
        b = pair(4,3)
        states = np.array([b,g,r,y])
        probs =np.zeros(4)
        t = 0
        for i in range(4):
            if self.drop_loc == (states[i].x,states[i].y):
                ...
                # probs[i] = 1/3        
        # print(probs)
        r_passeng = np.random.choice(states, p=probs)
        r_passenger = (r_passeng.x,r_passeng.y)
        for i in range(4):
            if self.drop_loc == (states[i].x, states[i].y) or r_passenger == (states[i].x, states[i].y):
                probs[i] = 0
            else:
                probs[i] = 1/2
                    
        r_tax = np.random.choice(states, p=probs)
        r_taxi = (r_tax.x,r_tax.y)
        return State(r_taxi, r_passenger, False, self.is_10_by_10)


    def get_random_depot_state_10(self):
        r = pair(0,0)
        g = pair(0,5)
        y = pair(8,0)
        b = pair(9,4)
        ...
        w = pair(3,3)
        m = pair(4,6)
        states = np.array([b,g,r,y,c,p,w,m])
        probs =np.zeros(8)
        t = 0
        for i in range(8):
            if self.drop_loc == (states[i].x,states[i].y):
...
class Agent:
    ...
    def execute_policy(self, policy):
        ...
        print(cur_state.val())
        num_iter = 0
        while not cur_state.finished and num_iter < 50:
            num_iter = num_iter + 1
            action = policy[cur_state.val()]
            print("Action Intended:",action, end = " | ")
            action = self.state.choose_action(action)
            print("Action Succeeded:", action, end = "  |  ")
            cur_reward = cur_state.get_reward(action)
            print("Reward:", cur_reward, end = " | ")
    ...
    def execute_policy_q(self, q_table):
        cur_state = State(self.state.taxi_loc, self.state.passenger_loc, self.state.passenger_sitting, self.state.is_10_by_10)
        total_reward = 0
        print(cur_state)
        print(cur_state.val())
        num_iter = 0
        while not cur_state.finished and num_iter < 100:
            num_iter = num_iter + 1
            action = self.index_to_action(np.argmax(q_table[cur_state.val()]))
    ...
    def getr_policy_q(self, q_table):
        cur_state = State(self.state.taxi_loc, self.state.passenger_loc, self.state.passenger_sitting, self.state.is_10_by_10)
        total_reward = 0
        num_iter = 0
        dr = GAMMA
        while not cur_state.finished and num_iter < 100:
            ...
            action = self.state.choose_action(action)
            cur_reward = cur_state.get_reward(action)
            total_reward = dr*cur_reward + total_reward
            dr *= GAMMA
            cur_state = cur_state.next_state(action)
        return total_reward            
    ...
    def q_learning(self, decay, num_episodes, q_table = False):
        """N, S, E, W, pickup, Drop"""
        if not q_table:
            self.q_table = {}
            for cur_state in self.state.possible_states:
        ...
        for episode_num in tqdm(range(num_episodes)):
            ...
            while True:
                ...
                if step > EPISODE_SIZE or next_state.finished:
                    break
            if decay:
                self.epsilon = self.epsilon / num_iter
    ...
    def sarsa_learning(self, decay, num_episodes, qvals = False):
        """N, S, E, W, pickup, Drop"""
        if not qvals:
            self.sarsa_table = {}
            for cur_state in self.state.possible_states:
        ...
        for episode_num in tqdm(range(num_episodes)):
            ...
            action_f, action_num_f = self.select_action(state_f, self.epsilon)
            total_reward = 0
            while True:
                reward = state_f.get_reward(action_f)
                state_s = state_f.next_state(action_f)
                action_s, action_num_s = self.select_action(state_s, self.epsilon)
    ...
    def get_d_rewards(self, method, new = True):
        num_episodes = 100
        if method == "q":
            if new == False:
                q_vals = self.q_learning(False, num_episodes, True)
            else:        
                q_vals = self.q_learning(False, num_episodes)
        elif method == "sarsa":
            if new == False:
                q_vals = self.sarsa_learning(False, num_episodes, True)
            else:        
                q_vals = self.sarsa_learning(False, num_episodes)
        elif method == "q_delay":
            if new == False:
                q_vals = self.q_learning(True, num_episodes, True)
            else:        
                q_vals = self.q_learning(True, num_episodes)
        elif method == "sarsa_delay":
            if new == False:
                q_vals = self.sarsa_learning(True, num_episodes, True)
            else:        
        ...
        for i in range(num_instances):
            ...
            d_factor = GAMMA
            total_reward = 0
            state = self.state.get_random_depot_state()
...
q_vals = agent.q_learning(False,1000)
...
#     for i in range(1,50):
#         if i==1:
#             vals.append(agent.get_d_rewards(method, True))
#         else:
#             vals.append(agent.get_d_rewards(method, False))
        
#         print("Reward = ", vals[-1])
#         plot(100 + 100* np.array(range(len(vals))), vals, "Plot of num_episodes vs Discounted rewards (" + method + ")", "Number of episodes", "Sum of Discounted Rewards", "ppplot-"+ method)
...
# state1 = State(STATE_R, STATE_Y, False, False)
# state2 = State(STATE_Y, STATE_R, False, False)
# state3 = State(STATE_B, STATE_Y, False, False)
# state4 = State(STATE_Y, STATE_B, False, False)
# states = [state1, state2, state3, state4]
# q_table = agent.q_learning(True, 2000)
# for st in states:
#     agent.state = st
#     agent.execute_policy_q(q_table)
#     print("\n\n\n\n")
...
#         print("Reward = ", vals[-1])
#         plot(100 + 100* np.array(range(len(vals))), vals, "num_episodes vs Discounted rewards alpha:"+str(ALPHA)+", self.epsilon:"+str(epsilon), "Number of episodes", "Sum of Discounted Rewards", "B4plot-("+str(ALPHA)+ ")("+ str(epsilon)+")")
...
# passenger_loc = STATE_Y                      
# taxi_loc = STATE_R
# state = State(taxi_loc, passenger_loc, False, True)

# for i in range(5):
#     ostate = state.get_random_depot_state()
#     agent = Agent(ostate)
#     ostate.initialize()
#     q_table = agent.q_learning(False, 10000)
#     rw = agent.getr_policy_q(q_table)
#     print(rw)